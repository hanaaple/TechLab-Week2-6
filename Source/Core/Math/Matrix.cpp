...
//mmmmmmmmmmmmmmmmm
// FMatrix FMatrix::GetRotationMatrix(const FVector& InEulerAngle)
// {
// 	// 각도를 라디안으로 변환
...
// 	);
//
// 	return (RollMatrix * PitchMatrix * YawMatrix).Transpose();
// }

FMatrix FMatrix::GetRotationMatrix(const FQuat& Q)
{
	// 쿼터니언 요소 추출
...
FMatrix FMatrix::LookAtLH(const FVector& EyePosition, const FVector& FocusPoint, const FVector& Up)
{
...
{
	FQuat q = GetQuaterion();

	return q.GetEuler();
}

FQuat FMatrix::GetQuaterion() const
{
